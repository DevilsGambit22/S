...
function showEmptyState() {
  ...
  miniCover.src = 'assets/images/default-cover.svg';
  syncButtons();
  syncPage();
}
...
function loadTrack(nextIndex, restore = true) {
  ...
  miniCover.src = track.cover || 'assets/images/default-cover.svg';
...
function syncPage() {
  ...
  if (track) {
    ...
    if (cover) cover.src = track.cover || 'assets/images/default-cover.svg';
  } else {
    ...
    if (cover) cover.src = 'assets/images/default-cover.svg';
  }
  ...
  if (!playlist.length) {
    ...
    list.innerHTML = playlist.map((track, trackIndex) => `
      <button class="track ${trackIndex === index ? 'active' : ''}" data-i="${trackIndex}" type="button">
        <img src="${escapeHtml(track.cover || 'assets/images/default-cover.svg')}" alt="">
